    ...
    <div style={{ maxWidth: '1040px', margin: '36px auto', padding: '0 24px 80px' }}>
      ...
      {nextAppt ? (
        ...
        <div className="glass-card" style={{ padding: '40px', textAlign: 'center', marginBottom: '40px' }}>
          ...
          <Link to="/" className="btn btn-fast-book" style={{ display: 'inline-flex', padding: '14px 28px' }}>
            <Zap size={18} fill="#FFFFFF" /> Find & Book Services
          </Link>
        </div>
      )}
      ...
      {past.length > 0 && (
        <div style={{ marginBottom: '44px' }}>
          <h2 style={{ fontSize: '1.4rem', fontWeight: 800, marginBottom: '20px' }}>Fast Rebook</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
            {past.slice(0, 2).map((b) => (
              <div key={b.id} className="glass-card glow-card" style={{ padding: '24px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-faint)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{b.businessName}</div>
                  <h3 style={{ fontSize: '1.2rem', fontWeight: 800, margin: '6px 0 8px' }}>{b.serviceName}</h3>
                  <p style={{ fontSize: '0.9rem', color: 'var(--brand-primary)', fontWeight: 800, fontFamily: 'Outfit, sans-serif' }}>₦{b.amount?.toLocaleString()} · {b.serviceDuration} mins</p>
                </div>
                <Link to={`/business/${b.businessSlug}`} className="btn btn-primary" style={{ marginTop: '20px', fontSize: '0.88rem' }}>
